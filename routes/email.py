...
def send_email_stream():
    ...
    try:
        ...
        thread = threading.Thread(
            target=email_service.send_email_massive_v1(config, data)
        )
        thread.start()
        return jsonify({"message": "El envío de correos ha comenzado"}), 200
...
def event_stream():
    while True:
        config = load_config(
            "outlook", "smtp", "drago_exchange_smtp.json"
        )  # Ruta al archivo de configuración SMTP
...
def emails():
    ...
    if sent_status not in ["sent", "received"]:
        return (
            jsonify({"error": "Invalid email type. Must be 'sent' or 'received'."}),
            400,
        )
